...
class JPypeSetup(object):
    ...
    def setupMacOSX(self):
        self.javaHome = '/System/Library/Frameworks/JavaVM.framework/Home'
        if not os.path.exists(self.javaHome):
            raise Exception('The JAVA_HOME variable must be defined.')

        #Check for the development headers for jpype
        headerDir = os.path.join('/System/Library/Frameworks/JavaVM.framework/','Headers/')
        if not os.path.exists(headerDir):
            raise Exception('The headers expected at %s were not found.'%(headerDir)+
                            'Install Java Developer package from http://connect.apple.com."'+
                            'In any case you must install the *.h files needed for jpype to work')
        self.macOSXJavaHeaders = headerDir
        self.jdkInclude = ""
        self.libraries = ["dl"]
        self.libraryDir = []
